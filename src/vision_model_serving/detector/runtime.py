...
class _LocalFocalNetDinoFactory:
    ...
    def build(self) -> object:
        try:
            ...
            if environment_spec.upstream_commits.get("focalnet_dino") != (
                self._expected_revision
            ):
                raise DetectorLoadError(
                    "artifact and source revision contracts do not match"
                )
        ...
        if not patch_results or any(
            result.state != "already_applied" for result in patch_results
        ):
        ...
        if not all(
            ...
            for name in ("load_state_dict", "to", "eval")
        ):
